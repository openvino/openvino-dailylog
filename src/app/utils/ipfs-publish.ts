...
async function publishToKlerosNode(fileName, data) {
  ...
  const url = `${environment.ipfsHost}/add`;
...
async function publishToTheGraphNode(fileName, data) {
  const url = `${environment.graphIpfsEndpoint}/api/v0/add?wrap-with-directory=true`;
  ...
  const result: any = await jsonStreamToPromise(response.body);
